...
func (wfe *WebFrontEndImpl) verifyPOST(ctx context.Context, logEvent *requestEvent, request *http.Request, regCheck bool, resource core.AcmeResource) ([]byte, *jose.JsonWebKey, core.Registration, *probs.ProblemDetails) {
	...
	//fmt.Printf("WFE.GO: el body vale %s FIN", body)
	//var rawRecurrent core.RecurrentRequest
        //err = json.Unmarshal(body, &rawRecurrent)
        //fmt.Printf("WFE.GO: El raw recurrent vale  %+v", rawRecurrent)
	//fmt.Println("------------------")
	//fmt.Println(body)
	// Verify JWS
	...
	fmt.Printf("WFE.GO: el payload vale %s FIN", payload)
	sizeOfPayload := len(payload)
	if strings.Contains(string(payload[:sizeOfPayload - 1]), "\"recurrent\": true") {
		subStr1 := strings.SplitN(string(payload[:sizeOfPayload - 1]), "recurrent_cert_validity\": ", 2)
        	subStr2 := strings.SplitN(subStr1[1], ",", 3)
		fmt.Printf("WFE.GO: recurrent_cer vale:substr1: %s substr2: %s LO ANTERIOR",subStr1, subStr2[0])
		//_, newErr := os.Create("STARValidityWFE")
		toFileErr := ioutil.WriteFile("STARValidityWFE", []byte(subStr2[0]), 0644)
		if toFileErr != nil {
			panic(toFileErr)
		}
	...
	if parsedRequest.Resource == "" {
        ...
        //fmt.Printf("WFE.GO: el body vale %s FIN", (string)payload)
