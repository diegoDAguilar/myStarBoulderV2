...
var (
	...
	var cfsslConfigObj cfsslConfig.Config
...
func NewCertificateAuthorityImpl(
	...
	//cfsslConfigObj.Signing.Profiles["rsaEE"].Expiry, _ = time.ParseDuration("24h")
	//cfsslConfigObj.Signing.Profiles["ecdsaEE"].Expiry, _ = time.ParseDuration("24h")
	internalIssuers, err := makeInternalIssuers(
		issuers,
		cfsslConfigObj.Signing,
		config.LifespanOCSP.Duration)
	//fmt.Printf("CA.GO: Internal issuers VALE: %v LO ANTERIOR", internalIssuers)
/*	for k:= range internalIssuers {
                fmt.Printf("THE key[%s] value[%s]\n", k, internalIssuers[k])
		//ccc,_:= x509.ParseCertificate(internalIssuers[k].cert)
		//fmt.Printf("EL CCC VALE:%s ", internalIssuers[k]
        }
*/
...
/*	for k := range ca.issuers {
                fmt.Printf("CAMPO key[%s] value[%s]\n FIN", k, ca.issuers[k])
                //fmt.Printf("Profiles mide : %v FIN", k)
        }
*/
	//cfsslConfigObj.Signing.Profiles["rsaEE"].Expiry, _ = time.ParseDuration("24h")
        //cfsslConfigObj.Signing.Profiles["ecdsaEE"].Expiry, _ = time.ParseDuration("24h")
	//fmt.Printf("-----------------------Buscando el campo %+v fin",ca.issuers["h2ppy h2cker fake CA"].eeSigner)
        //fmt.Println("La policiy vale: ")
	//myMap := ca.issuers["h2ppy h2cker fake CA"].eeSigner.Policy()
	//fmt.Printf("%+v", myMap.Profiles["rsaEE"].Expiry)


	return ca, nil
}
func (ca *CertificateAuthorityImpl) ChangeValidity (newVal string) {
	
	ca.validityPeriod, _ = time.ParseDuration(newVal)
...
/*func (ca *CertificateAuthorityImpl) ChangeConfig() {
	ca.Expiry = "24h"
        ca.CFSSL.Signing.Profiles["rsaEE"].ExpiryString = "24h"
        ca.CFSSL.Signing.Profiles["ecdsaEE"].ExpiryString = "24h"
...
func (ca *CertificateAuthorityImpl) IssueCertificate(ctx context.Context, csr x509.CertificateRequest, regID int64) (core.Certificate, error) {
	...
	//Checks if STARValidity exists, in that case it reads and then removes the file to apply STAR protocol
		...
		fmt.Printf("\nCA.GO: STAR protocol = true con crtExpStr => %s h", crtExpStr)
		myMap := ca.issuers["h2ppy h2cker fake CA"].eeSigner.Policy()
	...
	//const testing = "2017-08-24T10:45:00Z"
	//issuer.cert.NotAfter, _ = time.Parse(testing,"2017-08-24T10:45:00Z")
	...
	fmt.Printf("CA.GO: certPem contiene %+v", certPEM)
	ca.noteSignError(err)
	if err != nil {
	...
	//fmt.Printf("CA.GO: h2ppy cert vale: %+v FINFINFIN", ca.issuers["h2ppy h2cker fake CA"].cert)
	//fmt.Printf("CA.GO: h2ppy signer  vale: %s FINFINFIN", ca.issuers["h2ppy h2cker fake CA"].eeSigner.priv)
        //fmt.Printf("CA.GO: h2ppy signer  vale: %s FINFINFIN", ca.issuers["h2ppy h2cker fake CA"].eeSigner.ca)
	//fmt.Printf("CA.GO: todos los campos de ca: %+v", ca)
	...
	fmt.Printf("CA.GO:5 not before1 vale: %s FINFINFIN", ca.issuers["h2ppy h2cker fake CA"].cert.NotBefore)
        fmt.Printf("CA.GO:5 not after1  vale: %s FINFINFIN", ca.issuers["h2ppy h2cker fake CA"].cert.NotAfter)
        fmt.Printf("CA.GO:5 not before0 vale: %s FINFINFIN", ca.issuers["happy hacker fake CA"].cert.NotBefore)
        fmt.Printf("CA.GO:5 not after0  vale: %s FINFINFIN", ca.issuers["happy hacker fake CA"].cert.NotAfter)
	 for k:= range ca.issuers {
                fmt.Printf("THE key[%s] value[%s]\n", k, ca.issuers[k])
                //ccc,_:= x509.ParseCertificate(internalIssuers[k].cert)
                //fmt.Printf("EL CCC VALE:%s ", internalIssuers[k]
