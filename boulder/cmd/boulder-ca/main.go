...
func main() {
	...
	// _,_  = os.Create("~/STARValidity")
	// _, err2 := os.Stat("/root/gopath/src/github.com/letsencrypt/boulder/STARValidity3");
	//_, err2 := os.Stat("~/STARValidity");
	//if err2 == nil {
	//	c.CA.Expiry = "24h"
	//}
	
	/*for k := range c.CA.CFSSL.Signing.Profiles {
   		fmt.Printf("Campo key[%s] value[%s]\n", k, c.CA.CFSSL.Signing.Profiles[k])
		//fmt.Printf("Profiles mide : %v FIN", k)
	}*/
	
	//fmt.Printf("El quinto campo es %s FIN", c.CA.CFSSL.Signing.Profiles["rsaEE"].ExpiryString)
	//c.CA.CFSSL.Signing.Profiles["rsaEE"].ExpiryString = "24h"
	//c.CA.CFSSL.Signing.Profiles["ecdsaEE"].ExpiryString = "24h"
	/*for k := range c.CA.CFSSL.Signing.Profiles {
                fmt.Printf("Campo key[%s] value[%s]\n", k, c.CA.CFSSL.Signing.Profiles[k])
                //fmt.Printf("Profiles mide : %v FIN", k)
        }
	
	fmt.Printf("El quinto campo es %s FIN", c.CA.CFSSL.Signing.Profiles["rsaEE"])
	*/
	...
	/*for k := range cai.issuers {
                fmt.Printf("MAIN.GO: Campo key[%s] value[%s]\n", k, cai.issuers[k])
        }*/
	
	//fmt.Printf("MAIN.GO:2 %v FIN2", cai.issuers["h2ppy h2acker fake CA"])
	cai.PA = pa
	var tls *tls.Config
